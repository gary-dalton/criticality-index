# Experiment 01.04: Manna + Overtopping
...
- Storage: analogous to BTW. Path root `work/data/exp01_04/`.
...
If this experiment succeeds in locating the absorbing barrier for Manna, the next logical step is **A future coupled Manna + Overtopping experiment** — apply the same structural-fragility mechanism to the coupled-SOC setup from [`06_coupled_soc.md`](06_coupled_soc.md). This would test whether inter-system coupling can trigger structural failures in otherwise-stable systems (governance-relevant: does a neighbor's crisis destabilize you enough to push past the absorbing barrier?).
