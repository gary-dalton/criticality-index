# Experiment 01.03: Negative Controls — Signature Rejection on Non-SOC Systems
...
**`work/exp01_03_negatives.ipynb`** — same three-phase structure:
...
4. **Relationship to Exp 05.** Exp 05 (Suppression/Amplification) has Models D (genuine subcritical) and E (genuine supercritical) as controls for the distortion experiments. Is there overlap/duplication? Answer: Exp 05 runs Models D/E on BTW only for distortion-control purposes; Exp 01.03 runs them on both BTW and Manna as rejection-battery validation. Same mechanism, different scope. If we run 01.03 first, Exp 05 can reuse the data.
...
**Experiment 01.04: Manna + Overtopping** — see [`01_04_manna_overtopping.md`](01_04_manna_overtopping.md). Can run in parallel with 01.03 since they don't share infrastructure.

**Future validation-of-validation:** Once Exp 05 (Suppression/Amplification) runs, compare its Models D/E rejection results against 01.03's rejection results as a double-check — same physical regime, different experimental framings.
...
- [`../ideas/real_data_considerations.md`](../ideas/real_data_considerations.md) — Under-reporting artifacts. The rejection matrix from 01.03 is part of the disambiguation toolkit.
